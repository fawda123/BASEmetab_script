...
This document provides code and analysis to compare metabolism results from the Odum open-water method and BASEmetab with known results from the forward model included in the [Fwoxy package](https://github.com/jmarriola/fwoxy).  The Odum open-water method is available from the [sine branch](https://github.com/fawda123/WtRegDO/tree/sine) of the WtRegDO package, which includes a new option to output instantaneous metabolism flux estimates at each time step.  The BASEmetab results were created using [this script](https://github.com/fawda123/BASEmetab_script/blob/master/R/fwoxy_basemetab.R).  For BASEmetab, the reaeration coefficient was determined using the Wanninkhof method with the constant forcings from the Fwoxy model.  Also note that the BASEmetab model requires a PAR time series, which was created using the `fun_par_sin_model` from Fwoxy.  
...
Load the libraries, BASEmetab results, and setup model parameters and constants for the simulated dissolved oxygen time series from Fwoxy.
...
The metabolic output from WtRegDO, BASEmetab, and Fwoxy are then combined for a direct comparison. This requires a bit of additional wrangling, the details of which are not important.  The output shows the combined time series used to create the comparison plots. 
