...
This document provides code and analysis to compare metabolism results from the Odum open-water method, BASEmetab, and EBASE with known results from the forward model included in the [Fwoxy package](https://github.com/jmarriola/fwoxy).  The Odum open-water method is available from the [sine branch](https://github.com/fawda123/WtRegDO/tree/sine) of the WtRegDO package, which includes a new option to output instantaneous metabolism flux estimates at each time step.  The BASEmetab results were created using [this script](https://github.com/fawda123/BASEmetab_script/blob/master/R/fwoxy_basemetab.R).  For BASEmetab, the reaeration coefficient was determined using the Wanninkhof method with the constant forcings from the Fwoxy model.  Also note that the BASEmetab model requires a PAR time series, which was created using the `fun_par_sin_model` from Fwoxy.  The EBASE results were created using [this script](https://github.com/fawda123/BASEmetab_script/blob/master/R/fwoxy_ebase.R) and is similsr to BASEmetab with the exception of a parameterization for gas exchange following Wanninkhof 2014, i.e., gas exchange is estimated directly in the model and not calculated empirically outside of the Bayesian context.  An alternative and simpler version of EBASE was used that fixes the b parameter as part of hte Wanninkhof gas exchange estimation and is done using [this script](https://github.com/fawda123/BASEmetab_script/blob/master/R/fwoxy_ebase_fixed.R). 
...
Load the libraries, BASEmetab results, EBASE results (all parameters and fixed b), and setup model parameters and constants for the simulated dissolved oxygen time series from Fwoxy.
...
load(file = here('data/fwoxyebasefix.RData'))
...
# format EBASE fixed b to match fwoxy
ebfixmetab <- fwoxyebasefix %>% 
    mutate(
    secs = getsec(DateTimeStamp), 
    typ = 'EBASE, fixed b'
  ) %>% 
...
toplo1 <- example %>%
  ...
  gather(var, val, -secs, -typ) %>% 
  bind_rows(opmetab, bsmetab, ebmetab, ebfixmetab) %>% 
  mutate(
    var = factor(var, levels = c('D', 'Pg_vol', 'Rt_vol', 'dDO'), labels = c('gasexd', 'gppd', 'erd', 'dcdtd')), 
    typ = factor(typ, levels = c('Fwoxy', 'Odum', 'BASEmetab', 'EBASE', 'EBASE, fixed b'))
  )
...
The same comparison as above is made for the EBASE, fixed b results with Fwoxy. 

```{r}
 ggplot(toplo2, aes(x = Fwoxy, y = `EBASE, fixed b`, color = var)) + 
  geom_point() +
  facet_wrap(~var, ncol = 2, scales = 'free') +
  scale_color_manual(values = colors) + 
  theme_bw() + 
  geom_abline(intercept = 0, slope = 1) +
  labs(
    y = 'EBASE, fixed b, Flux, mmol/m3/d', 
    x = 'Fwoxy, Flux, mmol/m3/d'
  ) +
  theme(
    legend.title = element_blank(), 
...
For EBASE, we can compare the individual parameter estimates from values used to create the Fwoxy input.  The `a` parameter is light efficiency and was set at 0.2 mmol/m3/hr in the initial Fwoxy input. The estimate from EBASE is:
...
The value is an instantaneous measure at mmol/m3/15min.  To convert to mmol/m3/day, we multiply by 86400 / 900.

```{r}
ats * 86400 / 900
```

This is very close to the input used for Fwoxy. A second parameter output from Fwoxy is `b` as part of the gas exchange estimate.  A reasonable estimate for this value is 0.251 hr/cm.  The estimate from EBASE is: 
...
The value is an instantaneous measure at 15min/m. To convert to hr/cm, we multiply by 100 * 4 (or 400).

```{r}
bts * 400
```

This is about three times larger than what it should be, which is expected given that the gas exchange was over-estimated for EBASE.

Evaluating the same parameters for the EBASE, fixed b shows that they are estimated correctly. 
