...
output <- foreach(d = dates, .packages = c('here', 'R2jags'), .export = c('interval')) %dopar% { 
  ...
  metabfit <- do.call(R2jags::jags.parallel, 
                      list(data = data.list, inits = inits, parameters.to.save = params, model.file = here::here("ebase_model_fixed.txt"),
...
fwoxyebasefix <- do.call('rbind', output) %>% 
  na.omit() %>% 
...
save(fwoxyebasefix, file = here('data/fwoxyebasefix.RData'))
